...
生成Fmaf_summary.txt、Mmaf_summary.txt、Lmaf_summary.txt、Hmaf_summary.txt
...
p1 <- ggplot(data, aes(x = Type, y = h, color = Type)) +
  ...
  #stat_compare_means(aes(group = Type),   
                     method = "wilcox.test",   
                     comparisons = list(c("Significant QTL", "All QTL"),   
                                        c("Suggestive QTL", "All QTL")),   
                     label = "p.signif",   
                     label.y = c(0.95, 0.97)) 
p1
...
Python Variance_summary.py，生成 HLC_summary_variance.csv 和 FM_summary_variance.csv 文件 
...
for (prefix in file_prefixes) {
  ...
  cat("Residuals sum of squares of variance:", anova_table["Residuals", "Sum Sq"], "贡献度:", variance_components$Variance_Contribution[variance_components$Effect == "Residuals"], "\n")
